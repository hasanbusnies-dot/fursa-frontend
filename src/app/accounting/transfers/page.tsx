...
import { TRANSFER_METHOD_AR as METHOD_LABEL, TRANSFER_STATUS_AR, UI_AR } from '@/lib/staff-labels';

const STATUS_META: Record<string, { label: string; cls: string }> = {
  PENDING_VERIFICATION: { label: TRANSFER_STATUS_AR.PENDING_VERIFICATION, cls: 'bg-amber-100 text-amber-800' },
  CONFIRMED:            { label: TRANSFER_STATUS_AR.CONFIRMED,            cls: 'bg-green-100 text-green-700' },
  REJECTED:             { label: TRANSFER_STATUS_AR.REJECTED,             cls: 'bg-red-100 text-red-700'     },
  INITIATED:            { label: TRANSFER_STATUS_AR.INITIATED,            cls: 'bg-gray-100 text-gray-500'   },
};

const FILTERS: { value: TransferStatus | 'ALL'; label: string }[] = [
  { value: 'PENDING_VERIFICATION', label: TRANSFER_STATUS_AR.PENDING_VERIFICATION },
  { value: 'CONFIRMED',            label: TRANSFER_STATUS_AR.CONFIRMED },
  { value: 'REJECTED',             label: TRANSFER_STATUS_AR.REJECTED },
    ...
    <div className="bg-white border border-gray-200 rounded-2xl p-4 flex flex-col">
      ...
      {!pending && (row.confirmedByName || row.confirmedAt) && (
        <p className="mt-3 pt-3 border-t border-gray-100 flex items-center gap-1.5 text-[11px] text-gray-400">
          ...
          {row.confirmedByName ?? 'موظف'} · {formatDateTime(row.confirmedAt)}
        </p>
      )}
  ...
  const confirm = async (row: AccountingTransferRow) => {
    ...
    try {
      ...
      const bal = res.balanceAfter != null
        ? ` الرصيد الجديد: ${formatMoney(res.balanceAfter, res.currency ?? row.currency ?? 'SYP')}`
        : '';
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center gap-3">
          ...
          <p className="text-sm font-medium text-gray-500">
            {status === 'PENDING_VERIFICATION' ? 'لا توجد حوالات بانتظار الاعتماد.' : 'لا حوالات ضمن هذا الفلتر.'}
          </p>
        </div>
      ) : (
        <>
