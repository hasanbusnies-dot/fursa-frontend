...
export default function AdminTransfersPage() {
  ...
  const confirm = async (row: AdminTransferRow) => {
    ...
    try {
      ...
      const bal = res.balanceAfter != null ? ` Yeni bakiye: ${formatMoney(res.balanceAfter, res.currency ?? row.currency ?? 'SYP')}` : '';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center gap-3">
            ...
            <p className="text-sm font-medium text-gray-500">Onay bekleyen transfer yok.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {rows.map((r) => (
