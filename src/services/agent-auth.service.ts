...
// Agent auth (AP-M1 3-factor flow) uses RAW fetch — it MUST NOT go through api.ts.
// A 401 on /agent/login is not /auth/-prefixed, so the api interceptor would treat it
// as an expired access token, try to refresh (no token → fails), and hard-redirect to
// the consumer /login. Bypassing it keeps a bad-credentials error a plain error. Same
// pattern as token-refresh.ts / auth.store.ts (each defines its own BASE_URL).
...
export interface AgentMustChange {
  kind: 'mustChange';
  resetToken: string;
}

export type AgentLoginResult = AgentAuthed | AgentMustChange;
...
function extractAuthed(data: Record<string, unknown>): AgentAuthed {
...
export const agentAuthService = {
  /** 3-factor agent login. Returns either an authed result or a must-change-password
   *  result (first login with the one-time password). */
  login: async (payload: AgentLoginPayload): Promise<AgentLoginResult> => {
    const res = await fetch(`${BASE_URL}/agent/login`, {
      method: 'POST',
  ...
  firstSetPassword: async (newPassword: string, resetToken: string): Promise<AgentAuthed> => {
    const res = await fetch(`${BASE_URL}/agent/password/first-set`, {
      method: 'POST',
      headers: {
