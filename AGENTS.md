...
2. **Envelope rule:** the REST API returns `{ success, data }`; the **catalog endpoints return RAW arrays** — no envelope. Don't "fix" either side. `api.get/post/…` return the **raw parsed body, envelope and all** — they do NOT unwrap. Each `*.service.ts` unwraps `.data` itself and returns the clean shape to callers (`extractAuth` in `auth.service.ts` is the reference). So type the `api` call against the envelope (`api.get<{ data?: Thing }>`), never against `Thing`.
